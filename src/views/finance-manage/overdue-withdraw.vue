<template>
  <div class="mixin-components-container">
    <div class="filter-container">
      ...
      <el-button v-if="btnAry.findIndex(x => x.id == '166')!=-1" class="filter-item" type="primary" size="mini" @click="exportTable('1')">全部导出</el-button>
      <el-button v-if="btnAry.findIndex(x => x.id == '165')!=-1" class="filter-item" type="primary" size="mini" @click="exportTable('2')">按页导出</el-button>
    </div>
    ...
    <div>
      <el-table :data="list" border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column align="center" label="提现时间" width="160" prop="create_time" />
        ...
        <el-table-column label="操作" align="center" width="180">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status==1 && btnAry.findIndex(x => x.id == '161')!=-1" type="primary" size="mini" @click="remitList(scope.row)">打款</el-button>
            <el-button v-if="(scope.row.status==0 ||scope.row.status==2 ) && btnAry.findIndex(x => x.id == '160')!=-1" type="primary" size="mini" @click="checkList(scope.row)">审核</el-button>
            <el-button v-if="btnAry.findIndex(x => x.id == '162')!=-1" type="primary" size="mini" @click="detailList(scope.row)">查看详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination
      ...
      <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="结算单明细" name="second">
          <el-table :data="dataDetail" border style="width: 100%;margin-top:30px" >
            <el-table-column label="过期日期" prop="date" align="center" />
            <el-table-column align="left" label="酒店"  prop="hotel_name" />
            <el-table-column label="返还积分" prop="score" align="center" />
            <el-table-column label="状态" align="center" width="140">
              <template slot-scope="{row}">
...
import { getWithdrawList, getWithdrawDetailList, orderWithdrawListExcel,getMerchCheck,getMerchPayment } from '@/api/finance-manage/overdue-withdraw'
import mix from '@/mixs/mix'
import OverdueDetail from './components/overdue-detail'
...
export default {
  ...
  mounted() {
    this.getList()
    this.$store.dispatch('user/showBtn', { id: 159 }).then(res => {
      console.log('res', res)
  ...
  methods: {
    ...
    detailList(row) {
      ...
      const params = {
        divtime_score_expire_order_id: this.billDetail.divtime_score_expire_order_id,
